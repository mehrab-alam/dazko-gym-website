...
export default function DazkoHero() {
  ...
  const tabs = ["HOME", "ABOUT", "TRAINERS", "CLASSES", "SCHEDULES", "NEWS", "CONTACT"];
  ...
  return (
    <section className={`${styles.heroContainer} ${styles.dazkoHeroConatiner} dazkoHeroConatiner`}>
      ...
      <div className={styles.responsiveHeader}>
        <div>
          <img src='/img/dazko-img/Sak gym-log.png' alt='' />
        </div>
        <div
      ...
      <div className={styles.headerContent}>
        <div>
          Fee call <span>+1 234 456 78910</span>
        </div>
        <div>
      ...
      <div className={`${styles.tabTopBorder} ${isHamburger ? styles.borderActive : styles.borderInActive}`}></div>
      <div className={`${styles.tabContainer} tabContainer ${isHamburger ? styles.tabActive : styles.tabInActive}`}>
        <div className={styles.dazkoLogo}>
          <img src='/img/dazko-img/Sak gym-log.png' alt='' />
        </div>
        ...
        <div>
          <div>
            <Tabs tabBarStyle={{ color: "#fff", fontWeight: "600", fontSize: "1.2rem" }}>
              {tabs.map((tab, i) => {
                return <Tabs.TabPane key={i} tab={tab} />;
              })}
            </Tabs>
          </div>
          <span className={styles.memberBtn}>
            <Button>BECOME A MEMBER</Button>
            <span className={styles.angledBorder}></span>
          </span>
        </div>
      </div>
